...
>(({ className, defaultOpen = false, variant, ...props }, ref) => {
  const { isOpen, setIsOpen, shortcutText } = useCollapsibleState(defaultOpen);
  const contextKey=useUser().user?.id
  ...
  const THREAD_CONFIG = {
    labels: {
      ...
      closedState: "What you want to do ?",
    },
  };
  ...
  return (
    ...
    >
      ...
      />
      <Collapsible.Content>
        <div className="h-120 md:h-140 flex flex-col">
